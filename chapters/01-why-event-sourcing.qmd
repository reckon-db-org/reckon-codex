...
These corollaries together explain why event sourcing has overwhelming adoption in regulated industries and overwhelming non-adoption in places like simple CRUD applications. The value-per-event-stored is high when audit and forensics are load-bearing; low when they are not.
...
If a team is building a TODO-list app, a personal finance tracker, a simple blog, an internal admin panel — and the team is confident that nobody will need historical analysis — *do not use event sourcing*. The architectural overhead is real and unrewarded.
...
**Recursive: the building is itself event-sourced.** The work of *building software* — Inception through Rescue — is itself a process worth recording, and the Codex applies the architecture to itself. The Reckon Cycle (@sec-cycle) treats each stage as a Dossier; transitions between stages are events; build phases shelve and resume; the audit trail of *how the system came to be* is as available as the audit trail of what the system has done since.
